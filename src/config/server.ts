...
function verifyJWT2(req: any, res: any, next: any) {
  ...
  jwt.verify(token, SECRET_REDEFINICAO, (err: any, decoded: any) => {
    if (err) return res.status(401).json({
      error: `Invalid or Expired Token. Make sure you add a Header Parameter named x-access-token with the token provided when an email to reset password has been sent.`
    });
    req.userId = decoded.userId;
    next();
  });
...
//INTEGRAÇÃO PIX V2
var valordoPixMaquinaBatomEfi01 = 0;
var valordoPixPlaquinhaPixMP = 5000;
...
function converterPixRecebido(valorPix: number) {
  ...
  if (valorPix > 0 && valorPix >= ticket) {
    valorAux = valorPix;
    var creditos = Math.floor(valorAux / ticket);
...
function tempoOffline(data2: Date): number {
  ...
  return Math.abs((data2.getTime() - data1.getTime()) / 1000);
}
...
async function estornarMP(id: string, token: string, motivoEstorno: string) {
  ...
  try {
    ...
    if (numTentativasEstorno < 20) {
      await estornarMP(id, token, motivoEstorno);
    } else {
...
app.get("/consulta-maquina01", async (req, res) => {
  var pulsosFormatados = converterPixRecebido(valorDoPixMaquina01);
  valorDoPixMaquina01 = 0;
  ultimoAcessoMaquina01 = new Date();
...
app.get("/consulta-pix-efi-maq-batom-01", async (req, res) => {
  var pulsosFormatados = converterPixRecebido(valordoPixMaquinaBatomEfi01);
  valordoPixMaquinaBatomEfi01 = 0;
  if (pulsosFormatados != "0000") {
    return res.status(200).json({ "retorno": pulsosFormatados });
  } else {
    return res.status(200).json({ "retorno": "0000" });
...
function converterPixRecebidoMercadoPago(valorPix: number) {
  return ("0000000" + valorPix).slice(-7);
}
...
app.post("/rota-recebimento", async (req, res) => {
  try {
    ...
    console.log("Novo chamada a essa rota detectada:", req.body);
    
    if (req.body.pix && req.body.pix[0] && req.body.pix[0].txid == "70a8cacb59b53eac8ccb") {
...
app.post("/rota-recebimento-teste", async (req, res) => {
  try {
    console.log("Novo pix detectado:", req.body);
    
    if (req.body.txid == "flaksdfjaskldfjadfasdfccc") {
...
app.post("/rota-recebimento-mercado-pago", async (req: any, res: any) => {
  try {
    console.log("Novo pix do Mercado Pago:", req.body);
    ...
    console.log('transaction_amount', response.data.transaction_amount);
...
app.post("/maquina", verifyJwtPessoa, async (req: any, res) => {
  try {
    ...
    const condicoes: any[] = [
      { nome: req.body.nome, clienteId: req.body.clienteId }
    ];
    
    if (req.body.store_id) {
      condicoes.push({ store_id: req.body.store_id, clienteId: req.body.clienteId });
    }
    if (req.body.maquininha_serial) {
      condicoes.push({ maquininha_serial: req.body.maquininha_serial, clienteId: req.body.clienteId });
    }
    
    const maquinaExistente = await prisma.pix_Maquina.findFirst({
      where: { OR: condicoes },
      select: { id: true, nome: true, store_id: true, maquininha_serial: true }
    });
...
app.delete("/maquina", verifyJwtPessoa, async (req: any, res) => {
  try {
    ...
    await prisma.pix_Pagamento.deleteMany({
      where: { maquinaId: req.body.id },
    });
...
app.post("/credito-remoto", verifyJwtPessoa, async (req: any, res) => {
  try {
    ...
    if (maquina.cliente && !maquina.cliente.ativo) {
    ...
    if (NOTIFICACOES_CREDITO_REMOTO) {
      await notificarDiscord(NOTIFICACOES_CREDITO_REMOTO, `CRÉDITO REMOTO DE R$: ${req.body.valor}`, `Enviado pelo adm.`);
    }
...
app.post("/login-pessoa", async (req, res) => {
  try {
    ...
    const token = jwt.sign({ userId: user.id }, SECRET_PESSOA, { expiresIn: 3600 });
...
app.post("/login-cliente", async (req, res) => {
  try {
    ...
    if (user && user.dataVencimento) {
      ...
      } else if (diferencaEmDias > 5 && diferencaEmDias <= 10) {
  ...
  try {
    ...
    for (const maquina of maquinas) {
      ...
      if (maquina.ultimaRequisicao) {
        ...
        if (status == "ONLINE" && maquina.ultimoPagamentoRecebido && 
...
app.get("/clientes", verifyJwtPessoa, async (req: any, res) => {
  ...
  try {
    ...
    const clientesModificados = clientesComMaquinas.map(cliente => ({
      ...
      pagbankToken: cliente.pagbankToken ? "***********" + cliente.pagbankToken.slice(-3) : null,
    }));
...
app.post("/rota-recebimento-mercado-pago-dinamica/:id", async (req: any, res: any) => {
  try {
    ...
    if (!tokenCliente) {
      return res.status(400).json({ error: "Cliente sem token configurado" });
    }
    ...
    console.log('transaction_amount', response.data.transaction_amount);
    console.log('payment_type_id', response.data.payment_type_id);
    ...
    console.log("store id trazido pelo Mercado Pago...", str_id);
    
    if (response.data.status != "approved") {
      console.log(`Pagamento não aprovado ainda | Status MP: ${response.data.status}`);
      
      if (response.data.status === "pending") {
        console.log(`Iniciando checagem ativa para o pagamento ${req.query.id}...`);
        ...
        for (let tentativa = 1; tentativa <= 6; tentativa++) {
          ...
          try {
            ...
            console.log(`Tentativa ${tentativa}: status atual = ${statusFinal}`);
            
            if (statusFinal === "approved") {
              console.log(`✅ Pagamento ${req.query.id} confirmado mais rápido pelo polling!`);
        ...
        if (statusFinal !== "approved") {
          console.log(`⚠️ Pagamento ${req.query.id} não aprovado após checagens, aguardando webhook normal.`);
    ...
    if (maquina && maquina.store_id) {
      ...
      if (!cliente.ativo) {
        ...
        if (!registroExistente) {
          await estornarMP(req.query.id, tokenCliente, "cliente inativo");
      ...
      if (cliente.dataVencimento) {
        ...
        if (diferencaEmDias > 10) {
          ...
          if (!registroExistente) {
            await estornarMP(req.query.id, tokenCliente, "mensalidade com atraso");
      ...
      if (maquina.ultimaRequisicao) {
        ...
        if (diferencaEmSegundos > 60) {
          ...
          if (!registroExistente) {
            await estornarMP(req.query.id, tokenCliente, "máquina offline");
      ...
      if (valor < valorMinimo) {
        console.log("iniciando estorno por valor mínimo...");
        const registroExistente = await prisma.pix_Pagamento.findFirst({
          where: {
            mercadoPagoId: req.query.id,
            estornado: true,
        ...
        if (!registroExistente) {
          await estornarMP(req.query.id, tokenCliente, "valor inferior ao mínimo");
        ...
        return res.status(200).json({
          "retorno": `PAGAMENTO ESTORNADO - INFERIOR AO VALOR MÍNIMO DE R$: ${valorMinimo} PARA ESSA MÁQUINA.`
        });
      }
      ...
      if (NOTIFICACOES_PAGAMENTOS) {
        await notificarDiscord(NOTIFICACOES_PAGAMENTOS, 
          `Novo pagamento recebido no Mercado Pago. R$: ${valor.toString()}`, 
          `Cliente ${cliente?.nome} Maquina: ${maquina?.nome}. Descrição: ${maquina?.descricao || ''}`);
      }
      
      console.log('Pagamento inserido com sucesso:', novoPagamento);
      return res.status(200).json(novoPagamento);
    } else {
      console.log(`MÁQUINA NÃO POSSUI store_id CADASTRADO PARA O VALOR ${str_id}`);
      return res.status(200).json({ "retorno": "MÁQUINA NÃO ENCONTRADA" });
    }
  } catch (error: any) {
...
app.post("/webhookmercadopago/:id", async (req: any, res: any) => {
  try {
    ...
    // Código do webhook comentado por enquanto
    return res.status(200).json({ "status": "webhook recebido" });
...
app.post("/rota-recebimento-especie/:id", async (req: any, res: any) => {
  try {
    ...
    if (maquina) {
      ...
      return res.status(404).json({ "retorno": "error.. máquina não encontrado!" });
    }
  } catch (error) {
    console.error(error);
    return res.status(402).json({ "error": "error: " + error });
...
app.get("/pagamentos/:maquinaId", verifyJWT, async (req: any, res) => {
  ...
  try {
    ...
    for (const e of especie) {
      totalEspecie += parseFloat(e.valor);
    }
...
app.post("/relatorio-02-taxas", verifyJWT, async (req, res) => {
  try {
    ...
    for (const pagamento of pagamentos_pix) {
      totalTaxasPix += parseFloat(pagamento.taxas || "0");
    }
    ...
    for (const pagamento of pagamentos_credito) {
      totalTaxasCredito += parseFloat(pagamento.taxas || "0");
    }
    ...
    for (const pagamento of pagamentos_debito) {
      totalTaxasDebito += parseFloat(pagamento.taxas || "0");
    }
...
app.post("/relatorio-03-pagamentos", verifyJWT, async (req, res) => {
  try {
    ...
    let pagamentosPix = 0;
    for (const pagamento of pagamentos_pix) {
      pagamentosPix += parseFloat(pagamento.valor || "0");
    }
    ...
    for (const pagamento of pagamentos_credito) {
      pagamentosCredito += parseFloat(pagamento.valor || "0");
    }
    ...
    for (const pagamento of pagamentos_debito) {
      pagamentosDebito += parseFloat(pagamento.valor || "0");
    }
...
app.post("/relatorio-04-estornos", verifyJWT, async (req, res) => {
  try {
    ...
    const somatorioValores = pagamentos.reduce((acc, curr) => acc + parseFloat(curr.valor), 0);
...
var estornarOperacaoPagSeguroCount = 1;
...
app.post('/webhookpagbank/:idCliente', async (req: any, res: any) => {
  try {
    ...
    if (!tokenCliente || !emailCliente) {
      return res.status(400).json({ error: "Cliente sem token ou email configurado" });
    }
    ...
    if (transaction.deviceInfo && transaction.deviceInfo.length > 0) {
      ...
      const serialNumber = deviceInfo.serialNumber ? deviceInfo.serialNumber[0] : '';
      ...
      if (maquina && maquina.maquininha_serial) {
        ...
        if (!cliente.ativo) {
          ...
          if (!registroExistente) {
            await estornarOperacaoPagSeguro(emailCliente, tokenCliente, transaction.code[0].toString());
        ...
        if (NOTIFICACOES_PAGAMENTOS) {
          await notificarDiscord(NOTIFICACOES_PAGAMENTOS, 
            `Novo pagamento recebido no Pagbank. R$: ${transaction.grossAmount[0].toString()}`, 
            `Cliente ${cliente?.nome} Maquina: ${maquina?.nome}. Descrição: ${maquina?.descricao || ''}`);
        }
        ...
        if (NOTIFICACOES_GERAL) {
          await notificarDiscord(NOTIFICACOES_GERAL, 
            `Pagamento recebido em maquininha não cadastrada.`, 
            `Cliente ${cliente?.nome} Serial: ${serialNumber}.`);
        }
      }
    } else {
...
app.get('/is-client-ok/:id', async (req, res) => {
  try {
    ...
    if (cliente.dataVencimento) {
      ...
      return res.json({ status: diferencaEmDias <= 10 });
    } else {
      return res.status(400).json({ status: null });
    }
  } catch (error) {
...
app.get('/verificar-pagamento/:idCliente/:idPagamento', async (req, res) => {
  try {
    ...
    if (statusPagamento === 'approved') {
      var valor = response.data.transaction_amount;
      var tipoPagamento = response.data.payment_type_id;
      var taxaDaOperacao = "";
      ...
      const maquina = await prisma.pix_Maquina.findFirst({
        where: { id: response.data.external_reference },
        include: { cliente: true },
      });
      ...
      if (maquina) {
        if (!cliente.ativo) {
          ...
          if (!registroExistente) {
            await estornarMP(idPagamento, tokenCliente, "cliente inativo");
            await prisma.pix_Pagamento.create({
              data: {
                maquinaId: maquina.id,
                valor: valor.toString(),
        ...
        if (cliente.dataVencimento) {
          ...
          if (diferencaEmDias > 10) {
            ...
            if (!registroExistente) {
              await estornarMP(idPagamento, tokenCliente, "mensalidade com atraso");
              await prisma.pix_Pagamento.create({
                data: {
                  maquinaId: maquina.id,
                  valor: valor.toString(),
        ...
        if (maquina.ultimaRequisicao) {
          ...
          if (diferencaEmSegundos > 60) {
            ...
            if (!registroExistente) {
              await estornarMP(idPagamento, tokenCliente, "máquina offline");
              await prisma.pix_Pagamento.create({
                data: {
                  maquinaId: maquina.id,
                  valor: valor.toString(),
        ...
        if (valor < valorMinimo) {
          ...
          if (!registroExistente) {
            await estornarMP(idPagamento, tokenCliente, "valor inferior ao mínimo");
            await prisma.pix_Pagamento.create({
              data: {
                maquinaId: maquina.id,
                valor: valor.toString(),
        ...
        if (NOTIFICACOES_PAGAMENTOS) {
          await notificarDiscord(NOTIFICACOES_PAGAMENTOS, 
            `Novo pagamento recebido no Mercado Pago. Via APP. R$: ${valor.toString()}`, 
            `Cliente ${cliente?.nome} Maquina: ${maquina?.nome}.`);
        }

          ...
          if (response.data.status === "pending") {
            ...
            for (let tentativa = 1; tentativa <= 6; tentativa++) {
              ...
              try {
                ...
                console.log(
                  `Tentativa ${tentativa}: status atual = ${statusFinal}`
                );
                if (statusFinal === "approved") {
                  console.log(
                    `✅ Pagamento ${req.query.id} confirmado mais rápido pelo polling!`
                  );
            ...
            if (statusFinal !== "approved") {
              console.log(
                `⚠️ Pagamento ${req.query.id} não aprovado após checagens, aguardando webhook normal.`
              );
              return;
            }
          } else {
